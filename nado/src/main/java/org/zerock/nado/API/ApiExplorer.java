...
import org.zerock.nado.dto.SecurityEnvironmentDTO;
...
// 외교부_국가·지역별 재외공관 정보--------------------------------------------------------------------------------------------------------------------------------------------------------------
    public static List<EmbassyInfoDTO> getEmbassyList(String countryName) throws IOException {
        ...
        // JSON 응답을 EmbassyInfoDTO 리스트로 변환
        ObjectMapper objectMapper = new ObjectMapper();
        JsonNode rootNode = objectMapper.readTree(apiResponse);
        JsonNode dataNode = rootNode.get("data");

        List<EmbassyInfoDTO> embassyList = objectMapper.convertValue(
                dataNode,
                new TypeReference<List<EmbassyInfoDTO>>() {}
        );
    ...
    public static String getSecurityEnvironment(String countryName) throws IOException {
        ...
        List<SecurityEnvironmentDTO> securityEnvironmentDTOList = objectMapper.convertValue(
                dataNode,
                new TypeReference<List<SecurityEnvironmentDTO>>() {}
        );

        StringBuilder result = new StringBuilder();

        // securityEnvironmentList에 있는 데이터를 문자열로 결합
        for (SecurityEnvironmentDTO securityEnvironmentDTO : securityEnvironmentDTOList) {
            result.append(securityEnvironmentDTO.toString()); // 혹은 원하는 형식으로 데이터를 문자열로 만들기
            result.append("\n"); // 각 데이터를 줄바꿈으로 구분
        }
    ...
    // 외교부_국가∙지역별 특별여행주의보-----------------------------------------------------------------------------------------------------------------------------
    public static List<SptravelWarningListDTO> getSptravelWarningMap(String countryName) throws IOException {
        ...
        // 필요한 데이터 추출 및 문자열로 변환
        List<SptravelWarningListDTO> sptravelWarningListDTO = new ArrayList<>();

        for (JsonNode itemNode : dataNode) {
            SptravelWarningListDTO sptravelWarning = new SptravelWarningListDTO();
            sptravelWarning.setDangMapDownloadUrl(Collections.singletonList(itemNode.path("dang_map_download_url").asText()));
            // 다른 필요한 데이터도 동일하게 추가

            sptravelWarningListDTO.add(sptravelWarning);
        }

        return sptravelWarningListDTO;
    }
